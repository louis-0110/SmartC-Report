...
const inputValue = ref("请你根据我的工作产出为我生成一份日报。要求润色我的工作成果并为我制定明日工作计划。结果需要以列表的形式呈现。我的主要工作产出是:")
...
async function get_ai_log() {
  ...
  inputValue.value = logs.value.reduce((pre, cur) =>
    pre += cur + '\n'
    , '')
...
async function create_report() {
  ...
  const res = await getAiContent(inputValue.value);
  ai_result.value = res;
